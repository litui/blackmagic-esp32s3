...
void simple_uart_init(UartConfig* cfg) {
    ...
    uart_ll_set_sclk(UART_LL_GET_HW(cfg->uart_num), UART_SCLK_APB);
    // uart_hal_set_rxfifo_full_thr(UART_HAL(cfg->uart_num), UART_RX_FIFO_THRESH);
    uart_ll_set_rxfifo_full_thr(UART_LL_GET_HW(cfg->uart_num), UART_RX_FIFO_THRESH);
    // uart_hal_set_txfifo_empty_thr(UART_HAL(cfg->uart_num), UART_TX_FIFO_THRESH);
    uart_ll_set_txfifo_empty_thr(UART_LL_GET_HW(cfg->uart_num), UART_TX_FIFO_THRESH);
    // uart_hal_rxfifo_rst(UART_HAL(cfg->uart_num));
    uart_ll_rxfifo_rst(UART_LL_GET_HW(cfg->uart_num));
    // uart_hal_set_hw_flow_ctrl(UART_HAL(cfg->uart_num), UART_HW_FLOWCTRL_DISABLE, 0);
    uart_ll_set_hw_flow_ctrl(UART_LL_GET_HW(cfg->uart_num), UART_HW_FLOWCTRL_DISABLE, 0);
    // uart_hal_set_rx_timeout(UART_HAL(cfg->uart_num), 0x16);
    uart_ll_set_rx_tout(UART_LL_GET_HW(cfg->uart_num), 0x16);
    ...
    // disable interrupts
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RXFIFO_FULL);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_TXFIFO_EMPTY);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_PARITY_ERR);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_FRAM_ERR);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RXFIFO_OVF);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_DSR_CHG);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_CTS_CHG);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_BRK_DET);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RXFIFO_TOUT);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_SW_XON);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_SW_XOFF);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_GLITCH_DET);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_TX_BRK_DONE);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_TX_BRK_IDLE);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_TX_DONE);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RS485_PARITY_ERR);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RS485_FRM_ERR);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RS485_CLASH);
    uart_ll_clr_intsts_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_CMD_CHAR_DET);
    ...
    uart_ll_ena_intr_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RXFIFO_FULL);
    uart_ll_ena_intr_mask(UART_LL_GET_HW(cfg->uart_num), UART_INTR_RXFIFO_TOUT);   
...
uint32_t simple_uart_read(uint8_t uart_num, uint8_t* data, const uint32_t data_size) {
    const int num_rx = uart_hal_get_rxfifo_len(UART_HAL(uart_num));
    uint32_t read = ((data_size) < (num_rx) ? (data_size) : (num_rx));
    ...
    uart_ll_read_rxfifo(UART_LL_GET_HW(uart_num), data, read);
    return read;
}
...
void simple_uart_set_baud_rate(uint8_t uart_num, uint32_t baud_rate) {
    ...
    uart_ll_set_baudrate(UART_LL_GET_HW(uart_num), baud_rate, APB_CLK_FREQ);
}

void simple_uart_set_stop_bits(uint8_t uart_num, uart_stop_bits_t stop_bits) {
    uart_config[uart_num].stop_bits = stop_bits;
    uart_ll_set_stop_bits(UART_LL_GET_HW(uart_num), stop_bits);
...
void simple_uart_set_parity(uint8_t uart_num, uart_parity_t parity) {
    ...
    uart_ll_set_parity(UART_LL_GET_HW(uart_num), parity);
}
...
void simple_uart_set_data_bits(uint8_t uart_num, uart_word_length_t data_bits) {
    ...
    uart_ll_set_data_bit_num(UART_LL_GET_HW(uart_num), data_bits);
}
